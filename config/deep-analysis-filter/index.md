...
You receive complete numbered document chunks, one per `<entry>` message.

[entries/shape.md]

Each `<entry>` begins with a `<code>` child naming the single code to apply. The remaining lines are the complete chunk, numbered with sentence references. Each code's full definition is supplied separately in an `<analysis>` tag.
...
For every qualifying passage:

1. Apply only the `<code>` named in that entry.
...
The correct result may contain every entry, no entry, or any distribution in between. Judge each entry only from its text and definition.
